...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const mailOptions = {
      from: `"IvanGonzalez.cloud" <${import.meta.env.SMTP_USER}>`,
      to: 'hola@ivangonzalez.cloud',
      replyTo: email,
      subject: `🚀 Nuevo contacto (Astro): ${name}`,
    ...
    return new Response(
      JSON.stringify({ error: 'Error al enviar el email: ' + (error.message || 'Error desconocido') }),
      { status: 500 }
    );
  }
}
